...
MACRO (APPEND_TO_SERVER_SOURCES)
  FOREACH (src ${ARGN})
    APPEND_TO_CACHED_LIST(doxserver_sources
                          ${doxserver_sources_desc}
...
MACRO (APPEND_TO_SERVER_HEADERS)
  FOREACH (src ${ARGN})
    APPEND_TO_CACHED_LIST(doxserver_headers
                          ${doxserver_headers_desc}
                          ${CMAKE_CURRENT_SOURCE_DIR}/${src})
    APPEND_TO_CACHED_LIST(doxserver_headers_nopath
                          "doxserver_headers_nopath"
                          ${src})
  ENDFOREACH (src ${ARGN})
...
MACRO (APPEND_TO_SENSOR_SOURCES)
  FOREACH (src ${ARGN})
    APPEND_TO_CACHED_LIST(doxsensor_sources
                          ${doxsensor_sources_desc}
                          ${CMAKE_CURRENT_SOURCE_DIR}/${src})
  ENDFOREACH (src ${ARGN})
...
MACRO (APPEND_TO_CONTROLLER_SOURCES)
  FOREACH (src ${ARGN})
    APPEND_TO_CACHED_LIST(doxcontroller_sources
                          ${doxcontroller_sources_desc}
                          ${CMAKE_CURRENT_SOURCE_DIR}/${src})
  ENDFOREACH (src ${ARGN})
...
#################################################
macro (dox_add_executable _name)
  # Not defining STATIC or SHARED will use BUILD_SHARED_LIBS variable
  #foreach(ARG ${ARGN})
  file(GLOB_RECURSE SRCS "${ARGN}/*.cc")
      if(SRCS)
...
#################################################
macro (dox_install_includes _subdir)
  install(FILES ${ARGN} DESTINATION ${INCLUDE_INSTALL_DIR}/${_subdir} COMPONENT headers)
endmacro()

#################################################
macro (dox_install_library _name)
  set_target_properties(${_name} PROPERTIES SOVERSION ${CYBER_MAJOR_VERSION} VERSION ${CYBER_VERSION_FULL})
...
#################################################
macro (dox_install_executable _name)
  set_target_properties(${_name} PROPERTIES VERSION ${CYBER_VERSION_FULL})
  install (TARGETS ${_name} DESTINATION ${BIN_INSTALL_DIR})
endmacro ()

#################################################
macro (dox_setup_unix)
    # Using dynamic linking in UNIX by default
    set(BUILD_SHARED_LIBS TRUE)
endmacro()

#################################################
macro (dox_setup_windows)
...
#include (${dox_cmake_dir}/CyberTestUtils.cmake)
