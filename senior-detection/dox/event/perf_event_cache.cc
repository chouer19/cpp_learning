...
#include "dox/event/perf_event_cache.h"
...
void PerfEventCache::Shutdown() {
  ...
  of_ << dox::Time::Now().ToNanosecond() << std::endl;
  of_.flush();
  of_.close();
}
...
void PerfEventCache::Run() {
  EventBasePtr event;
  int buf_size = 0;
  while (!shutdown_ && !senior::dox::IsShutdown()) {
...
void PerfEventCache::Start() {
  auto now = Time::Now();
  std::string perf_file = "dox_perf_" + now.ToString() + ".data";
